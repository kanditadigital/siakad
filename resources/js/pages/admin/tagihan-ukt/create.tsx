...
export default function TagihanUktCreate({ mahasiswas, academicYearSemesters, uktSchemes }: Props) {
    ...
    const handleUktSchemeChange = (value: string) => {
        setData('ukt_scheme_id', value);
        if (value) {
            const scheme = uktSchemes.find((s) => s.id.toString() === value);
            if (scheme) {
                setData((prev) => ({ ...prev, ukt_scheme_id: value, jumlah_tagihan: scheme.jumlah.toString() }));
            }
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post('/admin/tagihan-ukt');
    };

    return (
        <>
            <Head title="Tambah Tagihan UKT" />
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold">Tambah Tagihan UKT</h1>
                        <p className="text-muted-foreground">Buat tagihan UKT baru</p>
                    </div>
                </div>

                <Card>
                    <CardHeader>
                        <CardTitle>Form Tambah Tagihan UKT</CardTitle>
                        <CardDescription>Isi data tagihan uang kuliah tunggal</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select value={data.ukt_scheme_id} onValueChange={handleUktSchemeChange}>
                                        <SelectTrigger>
                                    ...
                                    {errors.ukt_scheme_id && <p className="text-sm text-red-500">{errors.ukt_scheme_id}</p>}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    />
                                    {errors.jumlah_tagihan && <p className="text-sm text-red-500">{errors.jumlah_tagihan}</p>}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    />
                                    {errors.jumlah_bayar && <p className="text-sm text-red-500">{errors.jumlah_bayar}</p>}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    />
                                    {errors.jatuh_tempo && <p className="text-sm text-red-500">{errors.jatuh_tempo}</p>}
                                </div>
...
TagihanUktCreate.layout = (page: React.ReactNode) => (
    <AppLayout breadcrumbs={[
        { title: 'Dashboard', href: '/dashboard' },
        { title: 'Tagihan UKT', href: '/admin/tagihan-ukt' },
        { title: 'Tambah', href: '/admin/tagihan-ukt/create' },
    ]}>{page}</AppLayout>
);
