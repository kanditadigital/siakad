...
export default function PenjadwalanIndex({ kelases, mataKuliahs, dosens, ruangs, filters }: Props) {
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground">Kelola jadwal kelas perkuliahan</p>
                    </div>
                            ...
                            {kelases.data.length === 0 ? (
                                ...
                                kelases.data.map((kelas) => (
                                    <TableRow key={kelas.id}>
                                        ...
                                        <TableCell>{kelas.tahun_akademik}</TableCell>
                                        <TableCell><Badge variant={STATUS_VARIANTS[kelas.status] || 'outline'}>{kelas.status}</Badge></TableCell>
                                        <TableCell className="text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link href={`/admin/penjadwalan/${kelas.uuid}`}><Button variant="ghost" size="icon"><Eye className="h-4 w-4" /></Button></Link>
                                                <Link href={`/admin/penjadwalan/${kelas.uuid}/edit`}><Button variant="ghost" size="icon"><Edit className="h-4 w-4" /></Button></Link>
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        ...
                                                        <AlertDialogFooter>
                                                            ...
                                                            <AlertDialogAction onClick={() => handleDelete(kelas.uuid)} className="bg-red-600 hover:bg-red-700">Hapus</AlertDialogAction>
                                                        </AlertDialogFooter>
                                                    </AlertDialogContent>
                                                </AlertDialog>
                                            </div>
                ...
                {kelases.last_page > 1 && (
                    <div className="flex items-center justify-between">
                        <p className="text-sm text-muted-foreground">Menampilkan {kelases.data.length} dari {kelases.total} data</p>
                        <div className="flex items-center gap-2">
                            {Array.from({ length: kelases.last_page }, (_, i) => i + 1).map((page) => (
                                <Button key={page} variant={page === kelases.current_page ? 'default' : 'outline'} size="sm" onClick={() => router.get('/admin/penjadwalan', { ...filters, page })}>{page}</Button>
                            ))}
                        </div>
                    </div>
                )}
