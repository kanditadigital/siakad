...
export default function TendikIndex({ tendiks, jabatanList, filters }: Props) {
    ...
    const [jabatanFilter, setJabatanFilter] = useState(filters.jabatan || 'all');
        ...
        <>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">Data Tendik</h1>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <Select value={jabatanFilter} onValueChange={handleJabatanChange}>
                            ...
                            {tendiks.data.length === 0 ? (
                                ...
                                tendiks.data.map((tendik) => (
                                    <TableRow key={tendik.id}>
                                        ...
                                        <TableCell>{tendik.jabatan}</TableCell>
                                        <TableCell>{tendik.unit_kerja}</TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell className="text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link href={`/admin/tendik/${tendik.uuid}`}>
                                                ...
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        <AlertDialogHeader>
                                                            <AlertDialogTitle>Hapus Tendik</AlertDialogTitle>
                                                            <AlertDialogDescription>
                                                                Apakah anda yakin ingin menghapus tendik {tendik.nama}?
                                                            </AlertDialogDescription>
                                                        </AlertDialogHeader>
                                                        <AlertDialogFooter>
                                                            <AlertDialogCancel>Batal</AlertDialogCancel>
                                                            <AlertDialogAction
                                                                onClick={() => handleDelete(tendik.uuid)}
                ...
                {tendiks.last_page > 1 && (
                    <div className="flex items-center justify-between">
                        <p className="text-sm text-muted-foreground">
                            Menampilkan {tendiks.data.length} dari {tendiks.total} data
                        </p>
                        <div className="flex items-center gap-2">
                            {Array.from({ length: tendiks.last_page }, (_, i) => i + 1).map(
                                (page) => (
                                    <Button
                                        key={page}
                                        variant={page === tendiks.current_page ? 'default' : 'outline'}
                                        size="sm"
                                        onClick={() =>
                                            router.get('/admin/tendik', {
                                                ...filters,
