...
export default function ProgramStudiIndex({ programStudis, fakultas, filters }: Props) {
    const [search, setSearch] = useState(filters.search || '');
    const [fakultasFilter, setFakultasFilter] = useState(filters.fakultas || 'all');
        ...
        <>
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <Select value={fakultasFilter} onValueChange={handleFakultasChange}>
                            ...
                            {programStudis.data.length === 0 ? (
                                ...
                                programStudis.data.map((prodi) => (
                                    <TableRow key={prodi.id}>
                                        ...
                                        <TableCell>{prodi.nama_prodi}</TableCell>
                                        <TableCell>{prodi.fakultas}</TableCell>
                                        <TableCell>
                                            <Badge variant="outline">{prodi.jenis_prodi}</Badge>
                                        </TableCell>
                                        <TableCell>{prodi.lama_studi} Tahun</TableCell>
                                        <TableCell className="text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link href={`/admin/program-studi/${prodi.uuid}`}>
                                                    <Button variant="ghost" size="icon">
                                                        <Eye className="h-4 w-4" />
                                                    </Button>
                                                </Link>
                                                <Link href={`/admin/program-studi/${prodi.uuid}/edit`}>
                                                    <Button variant="ghost" size="icon">
                                                ...
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        <AlertDialogHeader>
                                                            ...
                                                            <AlertDialogDescription>
                                                                Apakah anda yakin ingin menghapus program studi {prodi.nama_prodi}?
                                                            </AlertDialogDescription>
                                                        </AlertDialogHeader>
                                                        <AlertDialogFooter>
                                                            <AlertDialogCancel>Batal</AlertDialogCancel>
                                                            <AlertDialogAction
                                                                onClick={() => handleDelete(prodi.uuid)}
                ...
                {programStudis.last_page > 1 && (
                    <div className="flex items-center justify-between">
                        <p className="text-sm text-muted-foreground">
                            Menampilkan {programStudis.data.length} dari {programStudis.total} data
                        </p>
                        <div className="flex items-center gap-2">
                            {Array.from({ length: programStudis.last_page }, (_, i) => i + 1).map(
                                (page) => (
                                    <Button
                                        key={page}
                                        variant={page === programStudis.current_page ? 'default' : 'outline'}
                                        size="sm"
                                        onClick={() =>
                                            router.get('/admin/program-studi', {
                                                ...filters,
