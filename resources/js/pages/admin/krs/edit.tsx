...
export default function KrsEdit({ krs, mahasiswas, kelases, academicYearSemesters }: Props) {
        ...
        <>
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold">Edit KRS</h1>
                        <p className="text-muted-foreground">Perbarui data KRS</p>
                    </div>
                </div>

                <Card>
                    <CardHeader>
                        <CardTitle>Form Edit KRS</CardTitle>
                        <CardDescription>Perbarui data Kartu Rencana Studi</CardDescription>
                    </CardHeader>
                    <CardContent>
                                        ...
                                        <SelectContent>
                                            {kelases.map((k) => (
                                                <SelectItem key={k.id} value={k.id.toString()}>
                                                    {k.kode_kelas} - {k.mata_kuliah?.nama_mk} ({k.mata_kuliah?.sks} SKS)
                                                    {k.dosen && ` | ${k.dosen.nama}`}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                    {errors.kelas_id && <p className="text-sm text-red-500">{errors.kelas_id}</p>}
                                </div>
                                        ...
                                        <SelectContent>
                                            {academicYearSemesters.map((ays) => (
                                                <SelectItem key={ays.id} value={ays.id.toString()}>
                                                    {ays.nama_tahun_akademik} - Semester {ays.semester}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
